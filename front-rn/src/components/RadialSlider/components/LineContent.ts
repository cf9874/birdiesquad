...
const LineContent = (props: ILineContentProps) => {
    const [markerPositionValues, setMarkerPositionValues] = useState<number[]>([])
    const {
        radius = 100,
        linearGradient = [],
        thumbBorderWidth = 5,
    ...
    return React.createElement(
        ...
        lines.map((_value, index) => {
            ...
            const activeIndex = ((((value - min) * 100) / (max - min)) * lineCount) / 100 + plusActiveIndex
            ...
            const markIndex = Math.floor(
                (((((!fixedMarker ? markerValue : value) - min) * 100) / (max - min)) * lineCount) / 100
            )
            ...
            const isSpeedoMeterMarkerLine =
                isRadialCircleVariant || isRadialSliderVariant || isSpeedoMeterVariant ? false : isMarkerLine
            const radialCircleLineRotation = isRadialCircleVariant ? 86 : 90
            return React.createElement(
                G,
                { key: index.toString() },
                (index % lineSpace === 0 || index === markIndex || isSpeedoMeterMarkerLine) &&
                    React.createElement(
                        G,
                        {
                            transform: `translate(${radius + (lineHeight - thumbBorderWidth)}, ${
                        ...
                        React.createElement(Line, {
                            ...
                            transform: `rotate(${index + radialCircleLineRotation + angle})`,
                            strokeWidth: 2,
                            stroke:
                                activeIndex > index || (index === markIndex && !isHideMarkerLine)
                                    ? Platform.OS === "web"
