...
const getExtraSize = (sliderWidth: number, thumbRadius: number, thumbBorderWidth: number) => {
    return Math.max(sliderWidth, (thumbRadius + thumbBorderWidth) * 2)
}
const getRadianByValue = (nvalue: number, openingRadian: number, max: number, min: number) => {
    return ((Math.PI - openingRadian) * 2 * (max - nvalue)) / (max - min) + openingRadian
}
...
const getCurrentRadian = (value: number, openingRadian: number, max: number, min: number) => {
